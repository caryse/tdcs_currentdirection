...
cfmResultFile = {'CP3FCZ_2mA_roastResult.mat', 'CPZFC3_2mA_roastResult.mat',...
    'C1FP2_2mA_roastResult.mat'};

%ROI
ROIfile = 'M1coord.mat'; %centre coordinates
ROIpatch = 'M1_bank_patch.mat'; %ROI patch (e.g. M1 bank)
%ROI + MONTAGE label - also becomes result output filename
ROIlabel = {'M1_CP3FCZ','M1_CPZFC3', 'M1_C1FP2'}; %M1_CP5FC1
...
%=================
%subjects
%NB: if folder only contains subject folders, edit end-3 to end.
cd(surfPATH)
k = dir; subj={k(3:end-3).name}'; %N={k(3:end-2).name}';
%subj = str2double(N);
%subj = {'subject2'}; %subject ID / MRI filename

%subjects equivalent name in roast (up until subject 53)
%num=1;
num=[1:52]; %number of total subjects with CFM
for i = 1:length(num)
    subj_cfm{i} = ['subject', num2str(num(i))];
end
subj_cfm = subj_cfm'; clear i
...
for mont = 1:length(cfmResultFile)
    for sub = 1:length(subj_cfm)
        ...
        if ~exist([ROIPATH, sprintf('%s/', subj{sub})], 'dir')
            continue
        else
        ...
        %=================
        surface = [surfPATH, sprintf('%s/%s/surf/%s.gii', subj{sub},subj{sub}, gmfilename)];
