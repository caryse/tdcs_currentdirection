...
for sub = 1:length(subj_cfm) %change to subj if subj_cfm not required
    ...
    cd([surfPATH, sprintf('%s/%s', subj{sub},subj{sub})]); %subject folder within surface path   
    ...
    if isfile(ROIfile) 
        ...
        figure = plot(sl);
        ROI_centre_coord_approx = input('define ROI centre as 3 numbers (e.g. -36 -28 56): ', 's');
